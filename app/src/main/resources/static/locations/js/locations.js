...
function fetchAndRenderLocations() {
    ...
    .then(response => {
        ...
        .then(responseData => {
            if (!response.ok) {
                showErrorMessage(responseData.message || "Failed to fetch locations.");
                return;
            }

            const locations = responseData.content
            const flattenedLocations = flattenObjects(locations);
            totalPages = responseData.page.totalPages;

            renderTable(flattenedLocations, "table-container", sampleObject, "/locations");
        })
    })
    .catch(err => {
...
function pollLocations() {
    fetchAndRenderLocations();
    setTimeout(pollLocations, 3 * 1000);
}
