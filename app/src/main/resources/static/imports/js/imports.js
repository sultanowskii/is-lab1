...
function fetchAndRenderImports() {
    let params;
    try {
    ...
    .then(response => {
        ...
        .then(responseData => {
            if (!response.ok) {
                showErrorMessage(responseData.message || "Failed to fetch imports.");
                return;
            }

            const imports = responseData.content
            const flattenedImports = flattenObjects(imports);
            totalPages = responseData.page.totalPages;

            renderTable(flattenedImports, "table-container", sampleObject, "/imports");
        })
    })
    .catch(err => {
